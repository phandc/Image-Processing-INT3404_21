...
patterns = ["$","price", "click", "retail", "sale", "link", "visit link", "discount", "product", "buy", "https:/", "http:/", "www"]
...
def isSpamEmail(email, checkingPattern):

    for pattern in checkingPattern:
        if(email.find(pattern) != -1):
            return True
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(directory):
        if filename.endswith("png"):
            ...
            textEmail = format_text(parsed_data)
            # print(textEmail)
            # print("----------------------------")
            # calling write_text function which will write arranged text into file
            # write_text(arranged_text)

            if (isSpamEmail(textEmail.lower(), patterns)):
                print("This is " + filename + " mail is spam")
                prediction.update({filename : "spam"})
            else:
                print("This is "+ filename + " mail is not spam")
                prediction.update({filename: "non-spam"})
    ...
    false_positive = sum(value == 'non-spam' for value in prediction.values())
    total_nonspam = 10
    print(false_positive/total_nonspam * 100)
